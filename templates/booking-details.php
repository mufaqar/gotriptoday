...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const totalPriceElements = document.querySelectorAll(".totalPrice");
    const vehicleInput = document.querySelector("#vehicle_type");
    const finalPriceInput = document.querySelector("#final_price");

    // Function to calculate and update total price (example logic)
    function calculateTotalPrice() {
        // Get base price from selected vehicle
        const activeVehicle = document.querySelector(".vehicle-option.active");
        if (!activeVehicle) return;

        const vehiclePrice = parseFloat(activeVehicle.getAttribute("data-price")) || 0;
        const basePrice = parseFloat(document.querySelector("#base_price").value) || 0;
        const totalPrice = basePrice + vehiclePrice;

        // Update visible total price elements
        totalPriceElements.forEach(el => {
            el.textContent = totalPrice.toLocaleString("en-US", {
                style: "currency",
                currency: "USD"
            });
        });
        ...
        if (finalPriceInput) finalPriceInput.value = totalPrice;
    }
    ...
    vehicleOptions.forEach(option => {
        option.addEventListener("click", function() {
            ...
            vehicleOptions.forEach(opt => {
                opt.classList.remove("active", "border-success", "bg-light");
    ...
    // ✅ On page load — set default vehicle (if already active)
    const defaultVehicle = document.querySelector(".vehicle-option.active");
    if (defaultVehicle && vehicleInput) {
        vehicleInput.value = defaultVehicle.getAttribute("data-name");
        calculateTotalPrice();
    }
});
